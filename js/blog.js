...
async function fetchArticles() {
  try {
    ...
    const validArticles = articles
      .map(validateAndSanitizeArticle)
...
function validateAndSanitizeArticle(article) {
  if (!validateArticle(article)) return null;
  
  return {
    title: sanitizeHTML(article.title).substring(0, 200), // Limiter la taille
    slug: article.slug.replace(/[^a-zA-Z0-9-_]/g, ''), // Slug sécurisé
    content: sanitizeHTML(article.content).substring(0, 50000), // Limiter le contenu
    summary: sanitizeHTML(article.summary).substring(0, 500),
    date: sanitizeHTML(article.date).substring(0, 20),
    tags: Array.isArray(article.tags) ? 
          article.tags.slice(0, 10).map(tag => sanitizeHTML(tag).substring(0, 50)) : []
  };
}
